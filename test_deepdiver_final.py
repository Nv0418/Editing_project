#!/usr/bin/env python3
"""
Test script for the final DeepDiver style with updated inactive text color
"""
...
def test_deepdiver_final():
    """Test the final DeepDiver style with darker inactive text"""
    
    print("🎨 Testing DeepDiver with updated inactive text color...")
    ...
    colors = style_instance.json_config['typography']['colors']
    print(f"📝 Active text color: {colors['active_text']}")
    print(f"📝 Inactive text color: {colors['inactive_text']}")
    ...
    # Output path
    output_file = output_dir / "deep_diver_FINAL_darker_text.mp4"
    
    # Render video
    print(f"🎯 Rendering final DeepDiver to {output_file}")
    composition.write_video(str(output_file), codec='libx264', fps=30)
    print(f"✅ DeepDiver final version test completed!")
    
    return True

if __name__ == "__main__":
    test_deepdiver_final()